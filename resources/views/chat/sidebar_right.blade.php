    ...
    <div class="row mr-0">
        @foreach ($offices as $office)
            @if ($user_type->id < 3 && $chat_room->users->where('user_type_id', $user_type->id)->where('office_id', $office->id)->count())
                ...
                <ul @if($user_type->id === 1)id="{{ $size }}-staff-{{ $office->id }}" @elseif ($user_type->id === 2) id="{{ $size }}-user-{{ $office->id }}" @endif class="mb-0 col-12 collapse show">
        ...
        @if ($user_type->id == 3 && $chat_room->users->where('user_type_id', 3)->count())
            ...
            <ul id="{{ $size }}-trial" class="col-12 mb-0 collapse show">
                @foreach ($chat_room->users as $user)
